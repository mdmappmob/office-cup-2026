...
const FIFA_RANKING: Record<string, number> = {
  Brasil: 5, Argentina: 1, França: 2, Espanha: 8, Inglaterra: 4,
  Portugal: 6, Alemanha: 10, Holanda: 7, Bélgica: 3, Uruguai: 11,
  Croácia: 9, Marrocos: 13, Japão: 18, México: 12, "Estados Unidos": 14,
  Senegal: 20, "Coreia do Sul": 23, Suíça: 15, Colômbia: 16,
  Equador: 28, Canadá: 33, Tunísia: 31, Egito: 34, Irã: 21,
  "Nova Zelândia": 104, "Cabo Verde": 66, "Arábia Saudita": 54,
  Iraque: 59, Noruega: 42, Argélia: 30, Áustria: 22, Jordânia: 70,
  "RD do Congo": 64, Uzbequistão: 74, Gana: 29, Panamá: 45,
  "África do Sul": 66, "República Tcheca": 36,
  "Bósnia-Herzegovina": 39, Qatar: 58, Haiti: 87, Escócia: 38,
  Paraguai: 44, Austrália: 25, Turquia: 26, "Costa do Marfim": 40,
  Curaçao: 81, Suécia: 24,
};
  ...
  for (const m of groupMatches) {
    if (!teams.has(m.home_team)) {
      teams.set(m.home_team, {
        team: m.home_team, flag: m.home_flag, group: m.group ?? "",
    ...
    if (!teams.has(m.away_team)) {
      teams.set(m.away_team, {
        team: m.away_team, flag: m.away_flag, group: m.group ?? "",
        mp: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, gd: 0, pts: 0,
...
export function buildRoundOf32(
  ...
  const add = (hType: R32Matchup["homeType"], hG: string, aType: R32Matchup["awayType"], aG: string) => {
    matchups.push({ homeType: hType, homeGroup: hG, awayType: aType, awayGroup: aG, r32MatchIndex: matchups.length });
  };
...
export function computeBracket(
  ...
  if (groupsFilled) {
    ...
    for (const m of r32Matchups) {
      const homeTeam = getTeamForSlot(m.homeGroup, m.homeType, standings, standings.thirdPlaceRanking);
      const awayTeam = getTeamForSlot(m.awayGroup, m.awayType, standings, standings.thirdPlaceRanking);
